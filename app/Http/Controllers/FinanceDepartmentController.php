...
class FinanceDepartmentController extends Controller
{
    ...
    public function displayDashboard(){
        ...
        return view('bursars.dashboard');
    }

    public function take_fees(){
        $students = Student::all();
        return view('bursars.take-fee', compact('students'));
    }
    ...
    public function displayInputForm(Request $request, $student_id){
        ...
        return view('bursars.student-details', compact('student'));
    }
    /**
     * Get course lists
     */
    public function courses()
    {
        $courses = Courses::all();
        $students = Student::all();
        return view('bursars.courses', compact('courses', 'students'));
    }

    /**
     * From course get fee structure
     */
    public function structure($id)
    {
        $course = Courses::find($id);
        $feestructure = FeeStructure::where('courses_id', $id)->get();
        return view('bursars.fees-structure', compact('feestructure', 'course'));
    }

    /**
     * Storing Fee Structure
     */
    public function storeStructure(Request $request)
    {
        ...
        $availability = FeeStructure::where('courses_id', $request->course_id)->where('year', $request->year)->where('semester', $request->semester)->get();

        if ($availability->isEmpty()) {
            # code...
            $structure = new FeeStructure();
            $structure->courses_id = $request->course_id;
            $structure->year = $request->year;
            $structure->semester = $request->semester;
            $structure->created_at = date('Y-m-d');
            $structure->save();

            return redirect()->back()->with('status', 'structure added successfully');
        }else {
            return redirect()->back()->with('status', $request->year.' '.'Is already defined');
        }
    ...
    public function createItem($id)
    {
        ...
        return view('bursars.fee-items', compact('structure'));
    }
    /**
     * Storing Fee Item
     */
    public function feeitem(Request $request, $id)
    {
        $this->validate($request, [
        ...
        $availability = FeeItem::where('fee_structure_id', $id)->where('name', $request->name)->get();
        if ($availability->isEmpty()) {
            # code...
            $item = new FeeItem();
            $item->fee_structure_id = $id;
            $item->courses_id = $feestructure->course->id;
            $item->name = $request->name;
            $item->amount = $request->amount;
            $item->status = $request->status;
            $item->created_at = date('Y-m-d');
            $item->save();
            return redirect()->back()->with('status', $request->name.' '.'added successfully');
        }else {
            # code..
            return redirect()->back()->with('status', 'Already aded');
        }
    }
...
' ;
   ...
   $output .='
    ...
    public function getCleanStudentsPDF($className, $reference_link){
        ...
        <p style="text-align: center;"><img src="assets/images/logo/logo.jpeg"  alt="logo here"/></p>
        <h2 style="text-align: center; ">WINTERFEL HIGH SCHOOL</h2>
        <p style="text-align: center; font-size: 17px;">P.O BOX 12345 WINTERFEL, GOT. Email:winterfelhigh@gmail.com</p>
